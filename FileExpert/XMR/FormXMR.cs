...
{
    public partial class FormXMR : FileExpert.FormParser
    {
        //Parse to show the data.
        ParserXMR parserXMR = new ParserXMR();
        ...
        protected void ParseXMRFile()
        {
            Result result = new Result();

            int fileSize = (int)fileStream.Length;

            //XMR file is small. We will read in all the data in once.

            Int64 currentProgress = 0;
            Int64 latestProgress = 0;
            while (result.Fine && canRun)
            {
                byte[] xmrData = new byte[fileSize];
                ...
                //All in;
                if (fileSize != fileStream.Read(xmrData, 0, fileSize))
                {
                ...
                if(result.Fine)
                {
                    ...
                    if(result.Fine)
                    {
                        DataStore dataStore = new DataStore(xmrData);
        ...
        private void FormXMR_Load(object sender, EventArgs e)
        {
        ...
        {
            parserXMR.ParseXMR(this.treeViewParser, this.rootNode, dataStore);
        }
    }
}
