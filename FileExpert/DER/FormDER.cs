...
{
    public partial class FormDER : FileExpert.FormParser
    {
        //Parse to show the data.
        ParserDER parserDER = new ParserDER();
        ...
        protected void ParseDERFile()
        {
            Result result = new Result();

            int fileSize = (int)fileStream.Length;

            //DER file is small. We will read in all the data in once.

            Int64 currentProgress = 0;
            Int64 latestProgress = 0;
            while (result.Fine && canRun)
            {
                byte[] DERData = new byte[fileSize];
                ...
                //All in;
                if (fileSize != fileStream.Read(DERData, 0, fileSize))
                {
                ...
                if(result.Fine)
                {
                    ...
                    if(result.Fine)
                    {
                        DataStore dataStore = new DataStore(DERData);
        ...
        private void FormDER_Load(object sender, EventArgs e)
        {
        ...
        {
            parserDER.ParseDER(this.treeViewParser, this.rootNode, dataStore);
        }
    }
}
